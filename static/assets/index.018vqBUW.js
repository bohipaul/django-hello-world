(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))i(t);new MutationObserver(t=>{for(const r of t)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function s(t){const r={};return t.integrity&&(r.integrity=t.integrity),t.referrerPolicy&&(r.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?r.credentials="include":t.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(t){if(t.ep)return;t.ep=!0;const r=s(t);fetch(t.href,r)}})();const d=window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"?"http://127.0.0.1:8000/api":"/api";class p{constructor(e=d){this.baseURL=e}async request(e,s={}){const i=`${this.baseURL}${e}`,t={headers:{"Content-Type":"application/json",...s.headers},...s};t.body&&typeof t.body=="object"&&(t.body=JSON.stringify(t.body));try{const r=await fetch(i,t);if(!r.ok){const o=await r.json().catch(()=>({}));throw new Error(o.detail||`HTTP ${r.status}: ${r.statusText}`)}return await r.json()}catch(r){throw console.error("API Request Error:",r),r}}async getParticipants(){return this.request("/participants/")}async getParticipant(e){return this.request(`/participants/${e}/`)}async createParticipant(e){return this.request("/participants/",{method:"POST",body:e})}async updateParticipant(e,s){return this.request(`/participants/${e}/`,{method:"PUT",body:s})}async deleteParticipant(e){return this.request(`/participants/${e}/`,{method:"DELETE"})}async getStats(){return this.request("/stats/")}}const l=new p;class m{constructor(e){this.container=e,this.participants=[],this.loading=!1}async load(){console.log("🔄 Chargement de la liste des participants..."),this.loading=!0,this.render();try{const e=await l.getParticipants();console.log("📥 Réponse API reçue:",e),this.participants=e.results||e,console.log("👥 Participants traités:",this.participants),console.log("🔢 Nombre de participants:",this.participants.length),this.render()}catch(e){console.error("❌ Erreur lors du chargement des participants:",e),this.renderError(e.message)}finally{this.loading=!1}}render(){if(this.loading){this.container.innerHTML=`
    ...
    `,this.attachEventListeners()}attachEventListeners(){document.getElementById("participant-form").addEventListener("submit",this.handleSubmit.bind(this))}async handleSubmit(e){if(e.preventDefault(),this.loading)return;this.loading=!0,this.errors={},this.render();const s=new FormData(e.target),i={prenom:s.get("prenom"),nom:s.get("nom"),email:s.get("email"),telephone:s.get("telephone")||null,entreprise:s.get("entreprise")||null,poste:s.get("poste")||null,actif:s.has("actif")};try{await l.createParticipant(i),e.target.reset(),this.showSuccess(),this.onSuccess&&this.onSuccess()}catch(t){console.error("Erreur lors de l'ajout:",t),t.message.includes("email")?this.errors={email:"Cet email est déjà utilisé"}:this.errors={general:t.message}}finally{this.loading=!1,this.render()}}showSuccess(){const e=document.createElement("div");e.className="fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50",e.textContent="Participant ajouté avec succès !",document.body.appendChild(e),setTimeout(()=>{e.remove()},3e3)}}let n,c;window.deleteParticipant=async a=>{if(confirm("Êtes-vous sûr de vouloir supprimer ce participant ?"))try{await l.deleteParticipant(a),n.load()}catch(e){alert("Erreur lors de la suppression : "+e.message)}};document.addEventListener("DOMContentLoaded",()=>{const a=document.querySelector("#app");a.innerHTML=`
  ...
  `;const e=document.getElementById("participant-form-container"),s=document.getElementById("participant-list-container");c=new u(e,()=>{n.load()}),n=new m(s),window.participantList=n,window.participantForm=c,c.render(),console.log("🔍 Test de l'API..."),l.getParticipants().then(i=>{console.log("✅ API Response:",i),console.log("📊 Nombre de participants:",i.results?.length||i.length)}).catch(i=>{console.error("❌ Erreur API:",i)}),n.load()});
